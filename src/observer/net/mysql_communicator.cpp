...
const uint32_t CLIENT_TRANSACTIONS  = 65535;        // Client knows about transactions.
...
RC MysqlCommunicator::read_event(SessionEvent *&event)
{
  ...
  if (ret != 0) {
    LOG_WARN(
        "failed to read packet header. length=%d, addr=%s. error=%s",
  ...
  if (ret != 0) {
    LOG_WARN(
        "failed to read packet payload. length=%d, addr=%s, error=%s",

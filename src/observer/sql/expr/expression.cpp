...
void AggreExpression::get_aggre_expression(Expression *expr, std::vector<AggreExpression *> &aggrfunc_exprs)
{
  switch (expr->type()) {
    case ExprType::AGGREGATION: {
      auto x = dynamic_cast<AggreExpression *>(expr);
      aggrfunc_exprs.push_back(x);
    } break;
    default: break;
  }
}

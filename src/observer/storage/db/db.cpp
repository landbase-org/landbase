...
RC Db::drop_table(const char *table_name)
{
  ...
  if (table == nullptr) {
    LOG_INFO("Table %s does not exist, so there is no need to process it", table_name);
...
RC Db::open_all_tables()
{
  ...
  for (const std::string &filename : table_meta_files) {
    ...
    if (opened_tables_.count(table->name()) != 0) {
      delete table;
      LOG_ERROR(
          "Duplicate table with difference file name. table=%s, the other filename=%s", table->name(), filename.c_str()
      );
      return RC::INTERNAL;
    }
